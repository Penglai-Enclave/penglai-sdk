...
static int enclave_ioctl_init(void)
{
  ...
  if(ret < 0)
  {
    printk("Enclave_driver: register enclave_dev failed!\n");
  ...
  if(!addr)
  {
    printk("KERNEL MODULE: can not get free page which order is 0x%x", DEFAULT_SECURE_PAGES_ORDER);
    ret = -1;
    goto deregister_device;
  }

  ret = SBI_CALL_2(SBI_SM_INIT, __pa(addr), 1 << (DEFAULT_SECURE_PAGES_ORDER + RISCV_PGSHIFT));
  if(ret < 0)
  {
    printk("KERNEL MODULE: sbi call mm_init is failed\n");
    goto deregister_device;
  } 

  printk("register_chrdev succeeded!\n");
  return 0;
