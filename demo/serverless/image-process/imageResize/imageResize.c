...
#define UNIX_DOMAIN_SELF "/tmp/resize_function"
#define UNIX_DOMAIN_TARGET "/tmp/rotate_function"
...
int sod_resize_main(unsigned long arg0, void* vaddr, unsigned long size)
{
	...
	debug_printf("[@%s] begin resize img\n", __func__);
	sod_img rz = sod_resize_image(imgIn, newWidth, newHeight);
	debug_printf("[@%s] after resize img\n", __func__);
...
int main(){
  debug_printf("server1 begin to run\n");
  /* Wait for invoked */
  ...
  debug_printf("Resize: Message from client (%d)) :%s\n",num,recv_buf); 
  ...
  sod_resize_main(0, shm, shm_size);
  ...
  debug_printf("Resize: Reply from server (%d)) :%s\n",num,recv_buf); 
