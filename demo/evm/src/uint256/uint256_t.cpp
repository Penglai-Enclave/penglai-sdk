#include "../../include/uint256/uint256_t.build"
#include <vector>
#include <cstring>
...
std::pair <uint256_t, uint256_t> uint256_t::divmod(const uint256_t & lhs, const uint256_t & rhs) const{
    ...
    int bits_diff = lhs.bits() - rhs.bits();
    uint256_t copyd = rhs << bits_diff;
    uint256_t adder = uint256_t(1) << bits_diff;
    if (copyd > qr.second){
        copyd = copyd >> 1;
        adder = adder >> 1;
    }
